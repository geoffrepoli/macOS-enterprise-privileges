        ...
        if ([appArguments showStatus]) {
                ...
                if ([privilegesApp expirationInterval] > 0) {
                    ...
                    [[privilegesApp currentUser] privilegesExpirationWithReply:^(NSDate *expire, NSUInteger remaining) {
                        ...
                        if (remaining > 0) {
                            ...
                            [self writeConsole:[NSString stringWithFormat:@"Administrator privileges expire in %@",
                                [MTPrivileges stringForDuration:remaining
                                                      localized:NO
                                                   naturalScale:NO
                                ]
                            ]];
            ...
            if ([[privilegesApp currentUser] useIsRestricted]) {
                ...
                if ([[privilegesApp enforcedPrivilegeType] isEqualToString:kMTEnforcedPrivilegeTypeAdmin]) {
                    [self writeConsole:@"Administrator privileges have been assigned by your administrator"];
                } else if ([[privilegesApp enforcedPrivilegeType] isEqualToString:kMTEnforcedPrivilegeTypeUser]) {
                    [self writeConsole:@"Standard user privileges have been assigned by your administrator"];
                }
                            ...
                            if (renewAdminPrivileges) {
                                ...
                                [[privilegesApp currentUser] renewAdminPrivilegesWithCompletionHandler:^(BOOL success) {
                                    ...
                                    if (success) {
                                        ...
                                        [self writeConsole:[NSString stringWithFormat:@"Administrator privileges have been renewed and will expire in %@",
                                            [MTPrivileges stringForDuration:[privilegesApp expirationInterval]
                                                                  localized:NO
                                                               naturalScale:NO
                                            ]
                                        ]];
